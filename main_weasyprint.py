...
def calculate_retirement_projection(current_age, retirement_age, current_balance, annual_contribution,
                                    growth_rate=0.06, inflation_rate=0.025):
...
def generate_pdf_report(name: str, age: int, projection: dict, file_path: str, chart_path: str):
    ...
    <html>
      ...
      <body>
        ...
        <img src="file://{chart_path}" width="600" />
      </body>
    </html>
    """
...
def send_email_with_attachment(to_email: str, subject: str, body: str, file_path: str):
    ...
    with open(file_path, 'rb') as f:
        msg.add_attachment(f.read(), maintype='application', subtype='pdf', filename='retirement_summary.pdf')
...
async def submit_form(data: RetirementForm):
    ...
    file_path = f"/tmp/{data.fullName.replace(' ', '_')}_report.pdf"
